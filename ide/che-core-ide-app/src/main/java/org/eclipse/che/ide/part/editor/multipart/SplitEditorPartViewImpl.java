...
public class SplitEditorPartViewImpl extends Composite implements SplitEditorPartView {
    ...
    private SplitEditorPartView child;
...
//    }
    ...
    public SplitEditorPartView split(IsWidget replica, Direction direction) {
        ...
        if (direction == VERTICALLY) {
            return splitVertically(replica);
        }

        if (direction == HORIZONTALLY) {
            return splitHorizontally(replica);
        }
        return null;
    }
    ...
    public void removeChild() {
        boolean isPresent = splitLayoutPanel.remove(child);
        Log.error(getClass(), "=== is present " + isPresent);
        splitLayoutPanel.add(specimenPanel);
        child = null;
    }

    private SplitEditorPartView splitVertically(IsWidget replica) {
//        Log.error(getClass(), "***** constraints.direction == VERTICALLY");

        int newSize = specimenPanel.getOffsetWidth() / 2;

        child = new SplitEditorPartViewImpl(replica, this);
        ...
        child.setNeighbor(specimen);

        splitLayoutPanel.addWest(newSpecimenPanel, newSize);
        splitLayoutPanel.add(child);
        specimenPanel.add(splitLayoutPanel);
    ...
    private SplitEditorPartView splitHorizontally(IsWidget replica) {
//        Log.error(getClass(), "***** constraints.direction == HORIZONTALLY");
        ...
        splitLayoutPanel.addNorth(newSpecimenPanel, newSize);
        splitLayoutPanel.add(child);
        specimenPanel.add(splitLayoutPanel);
        specimenPanel = newSpecimenPanel;
    ...
    public void removePartStack(@NotNull PartStack partStack) {
        ...
        if (child != null) {
            boolean isPresent = splitLayoutPanel.remove(specimenPanel);
...
//            }
        ...
        if (child == null) {
            Log.error(getClass(), "child == null");
            parent.removeChild();

        } else {
            Log.error(getClass(), "splitLayoutPanel NOT null");
        }
